 ...
 * W3jsMin
 *
 * Usage - W3jsMin::minify($js);
 * Usage - W3jsMin::minify($js, $options);
 * Usage - W3jsMin::minify($js, array('flaggedComments' => false));
 *
 * @package JShrink
 * @author Robert Hafner <tedivm@tedivm.com>
...
class W3jsMin
{
    ...
    /**
     * Flag to indicate if a semicolon should be added after the current character
     * (used to prevent tagged template literals after backticks by properly separating statements)
     *
     *  @var bool
     */
    protected $needsSemicolonAfter = false;
    ...
    protected function initialize($js, $options)
    {
        ...
        $this->last_char = "\n";
        $this->output = "";
        $this->needsSemicolonAfter = false;
    ...
    protected function echo($char) {
        ...
        // Check if we need to add a semicolon after this character (for backticks to prevent tagged template literals)
        if ($this->needsSemicolonAfter) {
            $this->output .= ';';
            $this->last_char = ';';
            $this->needsSemicolonAfter = false;
        }
            ...
            switch ($this->a) {
                ...
                case $stringType:
                    if ($stringType === '`') {
                        ...
                        while ($saveIndex < $this->len) {
                            $rawChar = $this->input[$saveIndex];
                            if ($rawChar === ' ' || $rawChar === "\t" || $rawChar === "\r" || $rawChar === "\n" || ($rawChar < "\x20")) {
                                $saveIndex++;
                                continue;
                            }
                            $nextNonWhitespace = $rawChar;
                            break;
                        }
                        if ($nextNonWhitespace !== false && preg_match('/^[a-zA-Z_$\pL]$/u', $nextNonWhitespace) === 1) {
                            $this->needsSemicolonAfter = true;
                        }
                    }
                    break 2;
